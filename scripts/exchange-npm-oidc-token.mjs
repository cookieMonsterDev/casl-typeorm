...
// Writes the exchanged token to a fixed path under RUNNER_TEMP rather than exporting it via
// $GITHUB_ENV: $GITHUB_ENV is only re-read by the Actions runner *between* steps, so a later
// workflow step (the semantic-release run) reads this fixed path back out via
// NPM_CONFIG_USERCONFIG instead.
